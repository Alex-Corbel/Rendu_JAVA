...
public class App {
	...
	public static void dspMainMenu() throws IOException {
		int response;
		boolean first = true;
		do {
			if ( !first ) {
			...
			System.out.println( "* 9 - Export des contacts (.csv)     *" );
	...
	private static void restoreContacts() throws IOException {
		int response = 0;
		int valeur = 1;
		int valeur2 = 1;
		try ( DirectoryStream<Path> ds = Files.newDirectoryStream( Paths.get( BOOK_BKP_DIR ), "*.ser" ) ) {
			System.out.println( "**************************************" );
			System.out.println( "Quel fichier souhaitez-vous restaurer ?" );
			for ( Path path : ds ) {
				System.out.println( valeur + " - Fichier : " + path.getFileName() );
				valeur += 1;
			}
			System.out.print( "*Votre choix : " );
			try { response = sc.nextInt(); }
			catch ( InputMismatchException e ) { response = -1; }
			finally { sc.nextLine(); }
		}
	...
	private static void exportContacts() throws IOException {
		String csvFile =  new SimpleDateFormat( "yyyy-MM-dd-hh-mm-ss" ).format( new Date() ) + ".csv";
		FileWriter writer = new FileWriter(csvFile);
		writer.append("Id" + ',' + "Name" + ',' + "Email" + ',' + "Phone" + ',' + "Type" );
		for ( Map.Entry<String, Contact> entry : book.getContacts().entrySet() ) {
			writer.append( "\n" + entry.getValue().getId() + "," + entry.getValue().getName() + "," + entry.getValue().getEmail() + "," + entry.getValue().getPhone() + "," + entry.getValue().getType() );
		}
		writer.flush();
		writer.close();
